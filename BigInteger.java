...
public class BigInteger{
  ...
  private List<Integer> value;
	...
	public BigInteger(String value1) {
		...
		//take digital part
		String a = value1.charAt(0) == '-' ? value1.substring(1) : value1;
		
		//every four characters parsed as a int
		value = new ArrayList<>();
		int i = a.length() - 4;
		while( i > -4) {
			value.add(Integer.parseInt(a.substring(i >= 0 ? i : 0, i + 4)));
			 i -= 4;
		}
	...
	public BigInteger add(BigInteger b) {
		...
		if(isNegative(b.value)) {
			return subtract(new BigInteger(toComplement(b.value)));
		}
		
		//align the digit
		int length = Math.max(value.size(), b.value.size());
		List<Integer> num1 = copyOf(value, length);
		List<Integer> num2 = copyOf(b.value, length);
		List<Integer> result = new ArrayList<>();
		
		int shift = 0;
		for(int i = 0; i < length - 1; i++) {
			int c = num1.get(i) + num2.get(i) + shift;
		...
		//complement, if positive number add 0, if negative number add 9999
		else {		
	...
	public BigInteger subtract(BigInteger b) {
		...
		if(isNegative(b.value)) {
			return add(new BigInteger(toComplement(b.value)));
		}
		...
		int shift1 = 0;
		for(int i = 0; i < length - 1; i++) {
			int c = num1.get(i) - num2.get(i) - shift1;
		...
		if(shift1 == 1) {
			...
			//when overflow on the positive integer subtract another positive integer, the result is zero
			else {		
				result.clear();
			}
		...
		//complement, if the number which the user input is a positive integer, and then add 0, if it is a negative integer, add 9999 
		else {		
	...
	//multiply
	private BigInteger multiply(int value1, int shift2) {
		List<Integer> result = new ArrayList<>();
